...
async function loadIndex(year:number):Promise<IndexRow[]>{try{const p=JSON.parse(await readFile(auditDataPathOrThrow("audit","bsd-atp-main-pbp-history",String(year),"results.json"),"utf8"));return Array.isArray(p)?p:[];}catch{return[];}}
...
async function fetchPbp(id:string|number,tally:PbpRetrievalTally){const r=await fetchPbpClassified(id,{userAgent:"tennis-truth-engine-task18b-atp-main/1.0"});recordOutcome(tally,r);return r.ok?r.payload:null;}
async function fetchCandidatePbp(rows:IndexRow[],tally:PbpRetrievalTally,concurrency=6){
 const out:Array<{row:IndexRow;payload:any}>=[];
 for(let offset=0;offset<rows.length;offset+=concurrency){
  const chunk=rows.slice(offset,offset+concurrency);
  out.push(...await Promise.all(chunk.map(async row=>({row,payload:row.match_id==null?null:await fetchPbp(row.match_id,tally)}))));
 }
 return out;
}
...
async function computeObservations(args:{p1:string;p2:string;asOfDate:string;context?:string|null}):Promise<{status:ObservationStatus;observations:any[]}>{
 ...
 const promise=(async():Promise<{status:ObservationStatus;observations:any[]}>=>{
  ...
  status.reason=observations.length?"Approved ATP Main PBP reconstructed through canonical match identity where metric-specific raw fields are satisfied."
    :retrievalReached?"No matching approved ATP Main PBP satisfied Task 18B field requirements."
