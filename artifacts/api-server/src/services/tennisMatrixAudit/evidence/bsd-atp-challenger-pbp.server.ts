...
async function loadIndex(y:number):Promise<IndexRow[]>{try{const p=JSON.parse(await readFile(auditDataPathOrThrow("audit","bsd-atp-challenger-pbp-history",String(y),"results.json"),"utf8"));return Array.isArray(p)?p:[];}catch{return[];}}
...
async function fetchPbp(id:string|number,tally:PbpRetrievalTally){const r=await fetchPbpClassified(id,{userAgent:"tennis-truth-engine-task18b-atp-challenger/1.0"});recordOutcome(tally,r);return r.ok?r.payload:null;}
...
async function computeObservations(args:{p1:string;p2:string;asOfDate:string;context?:string|null}):Promise<{status:ObservationStatus;observations:any[]}>{
 ...
 const promise=(async():Promise<{status:ObservationStatus;observations:any[]}>=>{
  ...
  status.reason=observations.length?"Approved ATP Challenger PBP reconstructed through canonical match identity where metric-specific raw fields are satisfied."
    :retrievalReached?"No matching approved ATP Challenger PBP satisfied Task 18B field requirements."
